...
import ControlFilterPanel from './ControlFilterPanel.vue';
import {
  CONTROL_FILTER_PARAM_LABEL_KEYS,
  getActiveControlFilterKeys,
  type ControlFilterParamKey,
} from './controlFilterToolbarKeys';
...
const activeFilterKeys = computed((): ControlFilterParamKey[] =>
  getActiveControlFilterKeys(toValue(props.table.filters) ?? {})
);
...
function removeFilterParam(key: ControlFilterParamKey) {
...
<template>
  <div class="relative flex flex-shrink-0 flex-wrap items-center gap-1.5" dir="ltr">
    <button
      ...
      :aria-label="t('control.toolbar-add')"
      :title="t('control.toolbar-add')"
      @click="props.onAdd?.()"
    >
    ...
    <button
      ...
      :aria-label="t('control.toolbar-export')"
      :title="t('control.toolbar-export')"
    ...
    >
      <div class="flex shrink-0 items-center gap-1">
        <button
          ...
          :aria-label="t('control.toolbar-filter')"
          :aria-expanded="filterOpen"
          :title="t('control.toolbar-filter')"
          @click="toggleFilter"
        >
        ...
        <button
          ...
          :aria-label="t('control.toolbar-clear-filters')"
          :title="t('control.toolbar-clear-filters')"
          @click="clearFiltersFromToolbar"
        >
          <span class="truncate">{{ t('control.toolbar-clear-filters') }}</span>
        </button>
      </div>
      <div
      ...
      >
        ...
        <div class="flex min-w-0 flex-wrap items-center gap-1">
          ...
          >
            <span class="min-w-0 truncate" :title="t(CONTROL_FILTER_PARAM_LABEL_KEYS[key])">{{
              t(CONTROL_FILTER_PARAM_LABEL_KEYS[key])
            }}</span>
            <button
              type="button"
              ...
              :aria-label="
                t('control.filter-badge-remove-aria', { label: t(CONTROL_FILTER_PARAM_LABEL_KEYS[key]) })
              "
              :title="
                t('control.filter-badge-remove-aria', { label: t(CONTROL_FILTER_PARAM_LABEL_KEYS[key]) })
              "
    ...
    <Teleport to="body">
      <Transition name="control-filter-pop">
        <div
          v-if="filterOpen"
      ...
      <Transition name="control-filter-pop">
        <div
          ...
          role="dialog"
          aria-modal="true"
          :aria-label="t('control.filter-panel-title')"
          @click.stop
        >
          <ControlFilterPanel
            :table="table"
...
<style scoped>
.control-filter-pop-enter-active,
.control-filter-pop-leave-active {
  transition: opacity 0.15s ease;
}
.control-filter-pop-enter-from,
.control-filter-pop-leave-to {
  opacity: 0;
}
</style>
