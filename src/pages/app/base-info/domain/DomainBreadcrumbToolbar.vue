...
import DomainFilterPanel from './DomainFilterPanel.vue';
import {
  DOMAIN_FILTER_PARAM_LABEL_KEYS,
  getActiveDomainFilterKeys,
  type DomainFilterParamKey,
} from './domainFilterToolbarKeys';
...
const activeFilterKeys = computed((): DomainFilterParamKey[] =>
  getActiveDomainFilterKeys(toValue(props.table.filters) ?? {})
);
...
function removeFilterParam(key: DomainFilterParamKey) {
...
<template>
  <div class="relative flex flex-shrink-0 flex-wrap items-center gap-1.5" dir="ltr">
    <button
      ...
      :aria-label="t('domain.toolbar-add')"
      :title="t('domain.toolbar-add')"
      @click="props.onAdd?.()"
    >
    ...
    <button
      ...
      :aria-label="t('domain.toolbar-export')"
      :title="t('domain.toolbar-export')"
    ...
    >
      <div class="flex shrink-0 items-center gap-1">
        <button
          ...
          :aria-label="t('domain.toolbar-filter')"
          :aria-expanded="filterOpen"
          :title="t('domain.toolbar-filter')"
          @click="toggleFilter"
        >
        ...
        <button
          ...
          :aria-label="t('domain.toolbar-clear-filters')"
          :title="t('domain.toolbar-clear-filters')"
          @click="clearFiltersFromToolbar"
        >
          <span class="truncate">{{ t('domain.toolbar-clear-filters') }}</span>
        </button>
      </div>
      <div
      ...
      >
        ...
        <div class="flex min-w-0 flex-wrap items-center gap-1">
          ...
          >
            <span class="min-w-0 truncate" :title="t(DOMAIN_FILTER_PARAM_LABEL_KEYS[key])">{{
              t(DOMAIN_FILTER_PARAM_LABEL_KEYS[key])
            }}</span>
            <button
              type="button"
              ...
              :aria-label="
                t('domain.filter-badge-remove-aria', { label: t(DOMAIN_FILTER_PARAM_LABEL_KEYS[key]) })
              "
              :title="
                t('domain.filter-badge-remove-aria', { label: t(DOMAIN_FILTER_PARAM_LABEL_KEYS[key]) })
              "
    ...
    <Teleport to="body">
      <Transition name="domain-filter-pop">
        <div
          v-if="filterOpen"
      ...
      <Transition name="domain-filter-pop">
        <div
          ...
          role="dialog"
          aria-modal="true"
          :aria-label="t('domain.filter-panel-title')"
          @click.stop
        >
          <DomainFilterPanel
            :table="table"
...
<style scoped>
.domain-filter-pop-enter-active,
.domain-filter-pop-leave-active {
  transition: opacity 0.15s ease;
}
.domain-filter-pop-enter-from,
.domain-filter-pop-leave-to {
  opacity: 0;
}
</style>
