...
import FrameworkFilterPanel from './FrameworkFilterPanel.vue';
import {
  FRAMEWORK_FILTER_PARAM_LABEL_KEYS,
  getActiveFrameworkFilterKeys,
  type FrameworkFilterParamKey,
} from './frameworkFilterToolbarKeys';
...
const activeFilterKeys = computed((): FrameworkFilterParamKey[] =>
  getActiveFrameworkFilterKeys(toValue(props.table.filters) ?? {})
);
...
function removeFilterParam(key: FrameworkFilterParamKey) {
...
<template>
  <div class="relative flex flex-shrink-0 flex-wrap items-center gap-1.5" dir="ltr">
    <button
      ...
      :aria-label="t('framework.toolbar-add')"
      :title="t('framework.toolbar-add')"
      @click="props.onAdd?.()"
    >
    ...
    <button
      ...
      :aria-label="t('framework.toolbar-export')"
      :title="t('framework.toolbar-export')"
    ...
    >
      <div class="flex shrink-0 items-center gap-1">
        <button
          ...
          :aria-label="t('framework.toolbar-filter')"
          :aria-expanded="filterOpen"
          :title="t('framework.toolbar-filter')"
          @click="toggleFilter"
        >
        ...
        <button
          ...
          :aria-label="t('framework.toolbar-clear-filters')"
          :title="t('framework.toolbar-clear-filters')"
          @click="clearFiltersFromToolbar"
        >
          <span class="truncate">{{ t('framework.toolbar-clear-filters') }}</span>
        </button>
      </div>
      <div
      ...
      >
        ...
        <div class="flex min-w-0 flex-wrap items-center gap-1">
          ...
          >
            <span class="min-w-0 truncate" :title="t(FRAMEWORK_FILTER_PARAM_LABEL_KEYS[key])">{{
              t(FRAMEWORK_FILTER_PARAM_LABEL_KEYS[key])
            }}</span>
            <button
              type="button"
              ...
              :aria-label="
                t('framework.filter-badge-remove-aria', { label: t(FRAMEWORK_FILTER_PARAM_LABEL_KEYS[key]) })
              "
              :title="
                t('framework.filter-badge-remove-aria', { label: t(FRAMEWORK_FILTER_PARAM_LABEL_KEYS[key]) })
              "
    ...
    <Teleport to="body">
      <Transition name="framework-filter-pop">
        <div
          v-if="filterOpen"
      ...
      <Transition name="framework-filter-pop">
        <div
          ...
          role="dialog"
          aria-modal="true"
          :aria-label="t('framework.filter-panel-title')"
          @click.stop
        >
          <FrameworkFilterPanel
            :table="table"
...
<style scoped>
.framework-filter-pop-enter-active,
.framework-filter-pop-leave-active {
  transition: opacity 0.15s ease;
}
.framework-filter-pop-enter-from,
.framework-filter-pop-leave-to {
  opacity: 0;
}
</style>
