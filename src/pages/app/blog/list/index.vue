...
const { data: products, error, isFetching, execute: refreshProducts } = useFetch(`${uri.admin.blog.list}`, {
  body: { type: 'blog',limit:1000}
});
...
const deleteBlog = async () => {
  ...
  try {
    ...
    // Call delete API
    const { data, error } = await useFetch(uri.admin.product.delete, {
      method: 'POST',
      body: { 
    ...
    await nextTick()
    // Refill list by calling API
    await refreshProducts();
  } catch (err) {
    console.error('Delete blog error:', err);
    // Refill list even if there's an error
    await refreshProducts();
  } finally {
    await nextTick()
    await refreshProducts();
...
<template>
  <div class="container mx-auto px-4 py-8 max-w-6xl">
    ...
    <ConfirmationDialog
        ...
        :deny="cancelDelete"
    >
      {{ t('message.delete-page-confirmation') }} "{{ currentBlog?.title || '' }}"?
    </ConfirmationDialog>
    ...
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
      <div>
        ...
        <p class="text-sm text-gray-600 dark:text-gray-400">
          {{ t('admin.blogs.manageYourBlogs') }}
        </p>
      </div>

      <div class="flex gap-2">
        <button
            @click="router.push({ name: 'app-blog-create' })"
...
<!--      </div>-->
    ...
    <div v-else-if="error" class="bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded relative" role="alert">
      ...
      <span class="block sm:inline">{{ error.message || t('admin.blogs.loadError') }}</span>
    </div>
    ...
    <div v-else-if="productList.length === 0" class="text-center py-12 bg-white dark:bg-gray-800 rounded-lg shadow">
      ...
      <h3 class="mt-2 text-lg font-medium text-gray-900 dark:text-white">{{ t('admin.blogs.noBlogs') }}</h3>
      <p class="mt-1 text-gray-500 dark:text-gray-400">
        <template v-if="debouncedSearch || statusFilter !== 'all' || stockFilter !== 'all'">
          {{ t('admin.blogs.noResultsWithFilters') }}
        </template>
        <template v-else>
          {{ t('admin.blogs.noBlogsDescription') }}
        </template>
      </p>
      <button
      ...
      <button
          v-else
          @click="router.push({ name: 'app-blog-create' })"
          @click.stop="router.push({ name: 'app-blog-detail', params: { slug: product.slug } })"
          class="mt-4 px-4 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2"
      >
        {{ t('admin.blogs.addNew') }}
      </button>
    ...
    <div v-else-if="viewMode === 'table'">
      <div class="bg-white dark:bg-gray-800 shadow overflow-hidden sm:rounded-lg">
        <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700 table-fixed">
          ...
          <tr dir="">
            <th scope="col" class="px-4 py-3 text-right text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider w-2/3">
              {{ t('admin.blog') }}
            </th>
            <th scope="col" class="px-4 py-3 text-right text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider w-1/6">
              {{ t('admin.status') }}
            </th>
            <th scope="col" class="px-4 py-3 text-right text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider w-1/6">
              {{ t('admin.actions') }}
          ...
          <tr v-for="product in paginatedProducts" :key="product.id" class="hover:bg-gray-50 dark:hover:bg-gray-700">
            <td class="px-4 py-4">
              <div class="flex items-center gap-3">
                <img class="h-16 w-16 rounded-md object-contain flex-shrink-0" :src="product.image?.src || product.thumbnail?.src" :alt="product.image?.alt || product.thumbnail?.alt" />
                <div class="min-w-0 flex-1 overflow-hidden">
                  <div class="text-sm font-medium text-gray-900 dark:text-white truncate max-w-[36ch]" :title="product.title">
                    {{ product.title }}
                  </div>
...
<!--            </td>-->
            ...
            <td class="py-4 whitespace-nowrap text-right text-sm font-medium">
              <div class="flex justify-center items-center gap-3">
                <button
                    @click.stop="router.push({ name: 'app-blog-detail', params: { slug: product.slug } })"
                    class="flex items-center gap-1 px-3 py-1.5 text-blue-600 dark:text-blue-400 hover:text-blue-900 dark:hover:text-blue-500 hover:bg-blue-50 dark:hover:bg-blue-900/20 rounded-md transition-colors"
                    :title="t('admin.edit')"
                >
                ...
                <button
                    ...
                    class="flex items-center gap-1 px-3 py-1.5 text-red-600 dark:text-red-400 hover:text-red-900 dark:hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-md transition-colors"
    ...
    <div v-if="totalPages > 1" class="flex flex-col sm:flex-row justify-between items-center mt-6 gap-4">
      <div class="text-sm text-gray-600 dark:text-gray-400">
        ...
        {{ productList.length }} {{ t('pagination.blogs') }}
      </div>
