...
import ComplianceFilterPanel from './ComplianceFilterPanel.vue';

const COMPLIANCE_FILTER_PARAM_LABEL_KEYS: Record<string, string> = {
  rule_id: 'task.filter-field-rule',
  enforcer: 'task.filter-field-liaison',
  section_id: 'task.filter-field-section',
};

function getActiveComplianceFilterKeys(filters: Record<string, unknown>): string[] {
  return Object.keys(filters).filter((key) => key in COMPLIANCE_FILTER_PARAM_LABEL_KEYS);
}
...
const activeFilterKeys = computed(() => {
  const filters = toValue(props.table.filters) ?? {};
  const keys = getActiveComplianceFilterKeys(filters);
  return keys;
});
...
<template>
  <div class="relative flex flex-shrink-0 flex-wrap items-center gap-1.5" dir="ltr">
    ...
    >
      ...
      >
        ...
        <div class="flex min-w-0 flex-wrap items-center gap-1">
          ...
          >
            <span class="min-w-0 truncate" :title="t(COMPLIANCE_FILTER_PARAM_LABEL_KEYS[key])">{{
              t(COMPLIANCE_FILTER_PARAM_LABEL_KEYS[key])
            }}</span>
            <button
              type="button"
              ...
              :aria-label="
                t('task.filter-badge-remove-aria', {
                  label: t(COMPLIANCE_FILTER_PARAM_LABEL_KEYS[key]),
                })
              "
              :title="
                t('task.filter-badge-remove-aria', {
    ...
    <Teleport to="body">
      <Transition name="compliance-filter-pop">
        <div
          v-if="filterOpen"
      ...
      <Transition name="compliance-filter-pop">
        <div
          ...
          role="dialog"
          aria-modal="true"
          :aria-label="t('compliance-dashboard.filter-panel-title')"
          @click.stop
        >
...
<style scoped>
.compliance-filter-pop-enter-active,
.compliance-filter-pop-leave-active {
  transition: opacity 0.15s ease;
}
.compliance-filter-pop-enter-from,
.compliance-filter-pop-leave-to {
  opacity: 0;
}
</style>
