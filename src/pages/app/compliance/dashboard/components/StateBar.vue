...
import { stateColors, stateLabels, stateOrder, toFa } from "../helpers"
import type { CountByState } from "../types"

const props = defineProps<{ data: CountByState[] }>()

const rows = computed(() =>
  [...props.data].sort((a, b) => stateOrder.indexOf(a.state) - stateOrder.indexOf(b.state)),
)
...
const option = computed(() => ({
  ...
  tooltip: {
    ...
    ...tooltipStyle,
    formatter: (p: { name: string; value: number }[]) => `${p[0].name}<br/>${toFa(p[0].value)} وظیفه`,
  },
  xAxis: {
    type: "value",
  ...
  yAxis: {
    type: "category",
    inverse: true,
    data: rows.value.map((d) => stateLabels[d.state]),
