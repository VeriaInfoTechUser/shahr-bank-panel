...
import {
  COMPLIANCE_OPS_FILTER_PARAM_LABEL_KEYS,
  getActiveComplianceOpsFilterKeys,
  type ComplianceOpsFilterParamKey,
} from './complianceOperationsFilterToolbarKeys';
...
const activeFilterKeys = computed((): ComplianceOpsFilterParamKey[] =>
  getActiveComplianceOpsFilterKeys(toValue(props.table.filters) ?? {})
);
...
function removeFilterParam(key: ComplianceOpsFilterParamKey) {
...
<template>
  <div class="relative flex flex-shrink-0 flex-wrap items-center gap-1.5" dir="ltr">
    ...
    >
      <div class="flex shrink-0 items-center gap-1">
        <button
          ...
          :aria-label="t('task.toolbar-filter')"
          :aria-expanded="filterOpen"
          :title="t('task.toolbar-filter')"
          @click="toggleFilter"
        >
        ...
        <button
          ...
          :aria-label="t('task.toolbar-clear-filters')"
          :title="t('task.toolbar-clear-filters')"
          @click="clearFiltersFromToolbar"
        >
          <span class="truncate">{{ t('task.toolbar-clear-filters') }}</span>
        </button>
      </div>
      <div
      ...
      >
        ...
        <div class="flex min-w-0 flex-wrap items-center gap-1">
          ...
          >
            <span class="min-w-0 truncate" :title="t(COMPLIANCE_OPS_FILTER_PARAM_LABEL_KEYS[key])">{{
              t(COMPLIANCE_OPS_FILTER_PARAM_LABEL_KEYS[key])
            }}</span>
            <button
              type="button"
              ...
              :aria-label="
                t('task.filter-badge-remove-aria', {
                  label: t(COMPLIANCE_OPS_FILTER_PARAM_LABEL_KEYS[key]),
                })
              "
              :title="
                t('task.filter-badge-remove-aria', {
    ...
    <Teleport to="body">
      <Transition name="compliance-ops-filter-pop">
        <div
          v-if="filterOpen"
      ...
      <Transition name="compliance-ops-filter-pop">
        <div
          ...
          role="dialog"
          aria-modal="true"
          :aria-label="t('compliance-page.filter-panel-title')"
          @click.stop
        >
          <TasksFilterPanel
            :table="table"
...
<style scoped>
.compliance-ops-filter-pop-enter-active,
.compliance-ops-filter-pop-leave-active {
  transition: opacity 0.15s ease;
}
.compliance-ops-filter-pop-enter-from,
.compliance-ops-filter-pop-leave-to {
  opacity: 0;
}
</style>
