...
function buildInitialValues(row: Record<string, unknown>): {
  ...
  return { answer_note, answer_choice };
}
...
const executorLabel = computed(() => getProgressExecutorLabel(props.row));
...
function buildPayload(
  row: Record<string, unknown>,
  vals: { answer_note: string; answer_choice: string },
  level: 'doing' | 'done'
) {
  const progress = getProgress(row);
  ...
  const answer_score = parsed ? parsed.score : '';
  const answer_value = parsed ? parsed.value : '';
...
async function runSubmit(level: 'doing' | 'done', vals: {
  answer_note: string;
  answer_choice: string;
}) {
...
<template>
  <div class="mt-4 border-t border-slate-200 pt-3 dark:border-darkmode-600">
    ...
    >
      {{ t('compliance-page.doing-section-title') }}
    </p>

    <p class="mb-3 text-xs font-medium text-slate-800 dark:text-slate-100">
      <span class="text-slate-500 dark:text-slate-400">{{
        t('compliance-page.doing-executor-prefix')
      }}</span>
      {{ executorLabel }}
    </p>
