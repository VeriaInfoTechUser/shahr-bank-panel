...
import { getProgress } from '../complianceStatusHelpers';
import { COMPLIANCE_PENDING_ASSIGNMENT_FORM_ID } from './compliancePendingAssignmentFormId';
...
async function onSubmit(values: Record<string, unknown>) {
  ...
  const progressIdRaw = progress?.id;
  const progressId =
    progressIdRaw != null && progressIdRaw !== ''
      ? Number(progressIdRaw)
      : null;
  ...
  const payload: Record<string, unknown> = {
    task_id: taskId,
    file_id: null,
    progress_id: Number.isFinite(progressId as number) ? progressId : null,
    level: 'todo',
    user_id: userId,
    company_id: null,
    answer_score: null,
  ...
  try {
    ...
    if (result?.result) {
      toast(t('compliance-page.referral-submit-success'), { type: 'success' });
      emit('success');
    } else {
      toast(
        String(result?.error?.message ?? t('compliance-page.referral-submit-error')),
        { type: 'error' }
      );
    }
  } catch (e) {
    toast(
      e instanceof Error ? e.message : t('compliance-page.referral-submit-error'),
      { type: 'error' }
    );
  } finally {
...
<template>
  <div class="mt-4 border-t border-slate-200 pt-3 dark:border-darkmode-600">
    ...
    <Form
      v-else
      :id="COMPLIANCE_PENDING_ASSIGNMENT_FORM_ID"
