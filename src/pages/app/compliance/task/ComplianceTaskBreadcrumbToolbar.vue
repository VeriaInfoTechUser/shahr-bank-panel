...
import ComplianceTaskFilterPanel from './ComplianceTaskFilterPanel.vue';
import {
  COMPLIANCE_TASK_FILTER_PARAM_LABEL_KEYS,
  getActiveComplianceTaskFilterKeys,
  type ComplianceTaskFilterParamKey,
...
const activeFilterKeys = computed((): ComplianceTaskFilterParamKey[] =>
  getActiveComplianceTaskFilterKeys(toValue(props.table.filters) ?? {})
);
...
function removeFilterParam(key: ComplianceTaskFilterParamKey) {
...
<template>
  <div class="relative flex flex-shrink-0 flex-wrap items-center gap-1.5" dir="ltr">
    <button
      ...
      :aria-label="t('compliance-task.toolbar-export')"
      :title="t('compliance-task.toolbar-export')"
    ...
    >
      <div class="flex shrink-0 items-center gap-1">
        <button
          ...
          :aria-label="t('compliance-task.toolbar-filter')"
          :aria-expanded="filterOpen"
          :title="t('compliance-task.toolbar-filter')"
          @click="toggleFilter"
        >
        ...
        <button
          ...
          :aria-label="t('compliance-task.toolbar-clear-filters')"
          :title="t('compliance-task.toolbar-clear-filters')"
          @click="clearFiltersFromToolbar"
        >
          <span class="truncate">{{ t('compliance-task.toolbar-clear-filters') }}</span>
        </button>
      </div>
      <div
      ...
      >
        ...
        <div class="flex min-w-0 flex-wrap items-center gap-1">
          ...
          >
            <span class="min-w-0 truncate" :title="t(COMPLIANCE_TASK_FILTER_PARAM_LABEL_KEYS[key])">{{
              t(COMPLIANCE_TASK_FILTER_PARAM_LABEL_KEYS[key])
            }}</span>
            <button
              type="button"
              ...
              :aria-label="
                t('compliance-task.filter-badge-remove-aria', { label: t(COMPLIANCE_TASK_FILTER_PARAM_LABEL_KEYS[key]) })
              "
              :title="
                t('compliance-task.filter-badge-remove-aria', { label: t(COMPLIANCE_TASK_FILTER_PARAM_LABEL_KEYS[key]) })
              "
    ...
    <Teleport to="body">
      <Transition name="compliance-task-filter-pop">
        <div
          v-if="filterOpen"
      ...
      <Transition name="compliance-task-filter-pop">
        <div
          ...
          role="dialog"
          aria-modal="true"
          :aria-label="t('compliance-task.filter-panel-title')"
          @click.stop
        >
          <ComplianceTaskFilterPanel
            :table="table"
...
<style scoped>
.compliance-task-filter-pop-enter-active,
.compliance-task-filter-pop-leave-active {
  transition: opacity 0.15s ease;
}
.compliance-task-filter-pop-enter-from,
.compliance-task-filter-pop-leave-to {
  opacity: 0;
}
</style>
