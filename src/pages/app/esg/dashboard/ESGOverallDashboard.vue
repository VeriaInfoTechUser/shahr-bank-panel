...
const scoreCards = computed(() => [
  ...
  {
    label: 'محیط‌زیست',
    score: env.value?.summary.avg_score ?? 0,
    icon: IconPlant2,
    color: COLORS.env.main,
    yoy: '+6.5',
  },
  {
    label: 'اجتماعی',
    score: soc.value?.summary.avg_score ?? 0,
    icon: IconUsers,
    color: COLORS.soc.main,
    yoy: '+4.3',
  },
  {
    label: 'حاکمیت',
    score: gov.value?.summary.avg_score ?? 0,
    icon: IconBuildingBank,
    color: COLORS.gov.main,
    yoy: '+8.1',
  },
])
...
const trendOpt = computed(() => ({
  ...
  series: [
    ...
    },
    {
      ...
      symbolSize: 5,
      smooth: true,
      lineStyle: {color: COLORS.env.main, width: 2, type: 'dashed'},
      itemStyle: {color: COLORS.env.main}
    },
    {
      name: 'اجتماعی',
      type: 'line',
      ...
      symbolSize: 5,
      smooth: true,
      lineStyle: {color: COLORS.soc.main, width: 2, type: 'dashed'},
      itemStyle: {color: COLORS.soc.main}
    },
    {
      name: 'حاکمیت',
      type: 'line',
      ...
      symbolSize: 5,
      smooth: true,
      lineStyle: {color: COLORS.gov.main, width: 2, type: 'dashed'},
      itemStyle: {color: COLORS.gov.main}
    },
  ],
}));
...
const workflowOpt = computed(() => ({
  ...
  series: [{
    ...
    label: {
      ...
      formatter: '{main|68%}\n{sub|پیشرفت}',
      rich: {
        main: {fontSize: 18, fontWeight: 600, color: '#1e293b', lineHeight: 24},
        sub: {fontSize: 10, color: '#94a3b8', lineHeight: 16}
      },
    },
    emphasis: {scale: false},
    data: [
...
const sectionRadarOpt = (section: ESGSection | undefined, color: string) => {
  ...
  return {
    ...
    series: [{
      type: 'radar',
      data: [{
        value: domains.map(d => d.avg_score), name: 'امتیاز',
...
const incompleteItems = computed(() => [
  {
    label: 'شاخص‌های ناقص',
    value:
        (gov.value?.summary.unanswered ?? 0) +
        (soc.value?.summary.unanswered ?? 0) +
        (env.value?.summary.unanswered ?? 0),
    icon: IconChartBar,
    color: COLORS.danger,
  },
  {
...
<template>
  <div v-if="dashboardData" dir="rtl" class="esg-overall space-y-5 pb-10">
      ...
      <div>
        ...
        <p class="mt-1 text-xs text-slate-500 dark:text-slate-400">
          <span class="inline-flex items-center gap-1">
            {{
              totalKpis
            }} شاخص · {{ (gov?.domains?.length ?? 0) + (soc?.domains?.length ?? 0) + (env?.domains?.length ?? 0) }} دامنه</span>
    ...
    <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-5">
      ...
      <div class="rounded-xl border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-700 dark:bg-slate-800">
        <p class="text-[11px] font-medium text-slate-500 dark:text-slate-400 flex items-center gap-1">
          <IconDatabase :size="20" class="text-blue-400 text-sm"/>
          تکمیل داده‌ها
        </p>
    ...
    <div class="grid gap-5 lg:grid-cols-12">
      ...
      <!-- ESG Score Trend -->
      <div
          class="lg:col-span-4 rounded-xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-700 dark:bg-slate-800">
        <div class="mb-1 flex items-center gap-2">
          <IconChartLine class="text-slate-400" :size="20"/>
      ...
      <!-- ESG Breakdown Donut -->
      <div
          class="lg:col-span-3 rounded-xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-700 dark:bg-slate-800">
        <div class="mb-1 flex items-center gap-2">
          <IconChartPie class="text-slate-400" :size="20"/>
      ...
      <!-- Top ESG Indicators -->
      <div
          class="lg:col-span-3 rounded-xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-700 dark:bg-slate-800 overflow-hidden">
        <div class="mb-3 flex items-center gap-2">
          <IconStars class="text-slate-400" :size="20"/>
        ...
        <div class="space-y-2.5">
          <div v-for="ind in topIndicators" :key="ind.code" class="flex items-center gap-2">
            <div class="h-6 w-6 flex-shrink-0 flex items-center justify-center rounded"
              ...
              <component
                  :is="sectionConfig[ind.section]?.icon"
                  class="w-3 h-3"
                  :style="{ color: sectionConfig[ind.section]?.color }"
              />
            </div>
            <span class="flex-1 text-[11px] text-slate-600 dark:text-slate-400 truncate"
                  :title="ind.title">{{ ind.title.slice(0, 28) }}…</span>
      ...
      <div
          class="lg:col-span-2 rounded-xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-700 dark:bg-slate-800">
        <div class="mb-3 flex items-center gap-2">
          <IconChartBar class="text-slate-400" :size="20"/>
        ...
        <div class="space-y-3">
          <div v-for="item in completenessData.slice(0, 7)" :key="item.label">
            <div class="flex items-center justify-between mb-1">
              ...
              <span class="text-[11px] font-semibold text-slate-800 dark:text-white ml-2">{{
    ...
    <div class="grid gap-5 lg:grid-cols-3">
      <div
          v-for="([label, sec, color, icon]) in [
      ['محیط‌زیست', env, COLORS.env.main, IconPlant2],
      ['اجتماعی', soc, COLORS.soc.main, IconUsers],
      ['حاکمیت', gov, COLORS.gov.main, IconBuildingBank],
    ]"
          :key="label"
      ...
      >
        <div class="mb-1 flex items-center gap-2">
          ...
          <span
              class="mr-auto inline-flex items-center gap-1 rounded-md px-2 py-0.5 text-[11px] font-semibold"
        ...
        <VChart
            :option="sectionRadarOpt(sec, color)"
            autoresize
    ...
    <div class="grid gap-5 lg:grid-cols-3">
      ...
      <div class="rounded-xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-700 dark:bg-slate-800">
        <div class="mb-3 flex items-center gap-2">
          <IconGitMerge class="text-slate-400" :size="20"/>
      ...
      <div class="rounded-xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-700 dark:bg-slate-800">
        <div class="mb-3 flex items-center gap-2">
          <IconBuildingCommunity class="text-slate-400" :size="20"/>
        ...
        <div class="space-y-4">
          ...
          <IconBuildingBank :style="{ color: u.color }" :size="20"/> {{ u.label }}
              </span>
            ...
            <div class="h-2 w-full overflow-hidden rounded-full bg-slate-100 dark:bg-slate-700">
              <div class="h-2 rounded-full transition-all duration-700"
                   :style="{ width: u.value + '%', background: u.color }"/>
            </div>
          </div>
        </div>
      ...
      <div class="rounded-xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-700 dark:bg-slate-800">
        <div class="mb-1 flex items-center gap-2">
          <IconCloud class="text-slate-400" :size="20"/>
        ...
        <div class="mt-3 grid grid-cols-3 gap-2 text-center text-[11px]">
          <div class="rounded-md bg-emerald-50 p-2 dark:bg-emerald-900/20">
            <p class="font-semibold text-emerald-700 dark:text-emerald-400">
              {{ (env?.all_kpis?.find(k => k.code === 'ENV-GHG-001')?.value ?? 1200).toLocaleString() }}</p>
          ...
          <div class="rounded-md bg-amber-50 p-2 dark:bg-amber-900/20">
            <p class="font-semibold text-amber-700 dark:text-amber-400">
              {{ (env?.all_kpis?.find(k => k.code === 'ENV-GHG-002')?.value ?? 2500).toLocaleString() }}</p>
          ...
          <div class="rounded-md bg-red-50 p-2 dark:bg-red-900/20">
            <p class="font-semibold text-red-700 dark:text-red-400">
              {{ (env?.all_kpis?.find(k => k.code === 'ENV-GHG-003')?.value ?? 8000).toLocaleString() }}</p>
    ...
    <div class="grid gap-5 lg:grid-cols-3">
      ...
      <div class="rounded-xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-700 dark:bg-slate-800">
        <div class="mb-3 flex items-center gap-2">
          <IconPlug class="text-slate-400" :size="20"/>
  ...
  "
          >
            ...
            <span
                class="text-center text-[10px] font-medium text-slate-700 dark:text-slate-300 leading-tight"
            >
    {{ ds.label }}
  </span>
    ...
    <span
        ...
        :style="{ background: ds.connected ? COLORS.ok : COLORS.gray }"
    />
    {{ ds.connected ? 'متصل' : 'قطع' }}
      ...
      <div class="rounded-xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-700 dark:bg-slate-800">
        <div class="mb-3 flex items-center gap-2">
          <IconAlertCircle class="text-slate-400" :size="20"/>
    ...
    background: item.color + '08'
      ...
      <div class="rounded-xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-700 dark:bg-slate-800">
        <div class="mb-3 flex items-center gap-2">
          <IconHistory class="text-slate-400" :size="20"/>
    ...
    <div class="grid gap-5 lg:grid-cols-3">
      <div
          v-for="([label, sec, color, icon, bgc]) in [
      ['محیط‌زیست', env, COLORS.env.main, IconPlant2, COLORS.env.light],
      ['اجتماعی', soc, COLORS.soc.main, IconUsers, COLORS.soc.light],
      ['حاکمیت', gov, COLORS.gov.main, IconBuildingBank, COLORS.gov.light],
    ]"
          :key="label"
      ...
      >
        ...
        >
          ...
          <span
              ...
              :style="{ color }"
          >
        {{ sec.summary.avg_score.toFixed(1) }}%
      </span>
        </div>

        <!-- Stats row -->
        <div
            class="grid grid-cols-3 divide-x divide-slate-100 border-b border-slate-100 dark:divide-slate-700 dark:border-slate-700"
        >
          <div class="p-3 text-center">
            <p class="text-lg font-bold text-slate-900 dark:text-white">
              {{ sec.summary.total_kpis }}
            </p>
          ...
          <div class="p-3 text-center">
            <p class="text-lg font-bold text-slate-900 dark:text-white">
              {{ sec.summary.completion.toFixed(0) }}%
            </p>
        ...
        <div class="space-y-2 p-4">
          <div
              v-for="d in sec.domains.slice(0, 5)"
              :key="d.code"
              class="flex items-center gap-2"
          >
        <span
            class="w-32 truncate text-right text-[11px] text-slate-500 dark:text-slate-400"
            :title="d.title"
        >
          {{ d.title }}
        </span>
            ...
            <div class="h-1.5 flex-1 overflow-hidden rounded-full bg-slate-100 dark:bg-slate-700">
              <div
                  ...
                  :style="{
              width: d.avg_score + '%',
              background: sc(d.avg_score)
            }"
              />
            </div>

            <span
                class="w-10 text-left text-[11px] font-semibold"
                :style="{ color: sc(d.avg_score) }"
            >
          {{ d.avg_score.toFixed(0) }}%
        </span>
          </div>

          <p
              v-if="sec.domains.length > 5"
              class="text-right text-[11px] text-slate-400"
          >
            + {{ sec.domains.length - 5 }} دامنه دیگر
          </p>
        </div>
      </div>
    </div>
