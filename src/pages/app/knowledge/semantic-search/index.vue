          ...
          <div class="rounded-xl border border-slate-200/60 bg-white shadow-sm dark:border-darkmode-700/60 dark:bg-darkmode-800">
            <div class="flex items-center justify-between border-b border-slate-100 px-5 py-3 dark:border-darkmode-600">
              ...
              <Button
                ...
                @click="goToStep2"
              >
                <Lucide icon="ArrowLeft" class="me-1.5 h-3.5 w-3.5" />
                {{ t('semantic-search.next') }} ({{ selectedItems.length }})
              </Button>
            </div>
              ...
              >
                <div class="flex items-start gap-3">
                  <input
                    ...
                    class="mt-1 h-4 w-4 rounded border-slate-300 text-primary focus:ring-primary/20 dark:border-darkmode-500"
                    @click.stop
        ...
        <div class="rounded-xl border border-slate-200/60 bg-white shadow-sm dark:border-darkmode-700/60 dark:bg-darkmode-800">
          <div class="flex items-center justify-between border-b border-slate-100 px-5 py-3 dark:border-darkmode-600">
            <div class="flex items-center gap-3">
              <button
                type="button"
                class="inline-flex items-center gap-1 rounded-md px-2 py-1 text-xs font-medium text-slate-500 transition-all hover:bg-slate-100 hover:text-slate-700 dark:hover:bg-darkmode-600 dark:hover:text-slate-300"
                @click="backToStep1"
              >
                <Lucide icon="ArrowRight" class="h-3.5 w-3.5" />
                {{ t('semantic-search.back-to-results') }}
              </button>
              <span class="text-xs font-semibold text-slate-500 dark:text-slate-400">
                {{ selectedItems.length }} {{ t('semantic-search.selected-items') }}
              </span>
            </div>
          </div>

          <!-- Selected items summary -->
          <div class="border-b border-slate-100 px-5 py-3 dark:border-darkmode-600">
            <div class="flex flex-wrap gap-2">
              <span
                v-for="item in selectedItems"
                :key="item.chunkSlug"
                class="inline-flex items-center gap-1 rounded-full border border-primary/15 bg-primary/5 px-2.5 py-1 text-[11px] font-medium text-primary dark:border-primary/20 dark:bg-primary/10 dark:text-primary"
              >
                <span class="line-clamp-1 max-w-[200px]">{{ item.contentPreview.slice(0, 50) }}...</span>
                <button type="button" class="ms-0.5 hover:text-primary/70" @click="removeFromSelected(item)">
          ...
          <!-- Prompt select -->
          <div class="p-5">
            <label class="mb-1.5 block text-xs font-medium text-slate-500 dark:text-slate-400">{{ t('semantic-search.select-prompt') }}</label>
            <select
              v-model="selectedPromptSlug"
              class="w-full rounded-xl border border-slate-200 bg-slate-50 px-3 py-2.5 text-sm text-slate-800 transition-all focus:border-primary focus:bg-white focus:outline-none focus:ring-2 focus:ring-primary/20 dark:border-darkmode-600 dark:bg-darkmode-700 dark:text-slate-100"
            >
              <option value="" disabled>{{ t('semantic-search.prompt-placeholder') }}</option>
              <option v-for="p in promptOptions" :key="p.value" :value="p.value">
                {{ p.label }}
              </option>
            </select>
            ...
            <div class="mt-4 flex justify-end">
              ...
              >
                <Lucide v-if="generating" icon="Loader2" class="me-1.5 h-3.5 w-3.5 animate-spin" />
                <Lucide v-else icon="Sparkles" class="me-1.5 h-3.5 w-3.5" />
      ...
      <div class="col-span-12">
        <div class="rounded-xl border border-slate-200/60 bg-white shadow-sm dark:border-darkmode-700/60 dark:bg-darkmode-800">
          ...
          <div class="p-5">
            <!-- Prompt text -->
            <div class="mb-4">
              <span class="mb-1.5 block text-xs font-medium text-slate-400 dark:text-slate-500">{{ t('semantic-search.prompt-text') }}</span>
              <div class="rounded-xl border border-slate-200 bg-slate-50 p-3 dark:border-darkmode-600 dark:bg-darkmode-700">
                <pre class="whitespace-pre-wrap break-words text-xs leading-relaxed text-slate-700 dark:text-slate-300">{{ selectedPromptText }}</pre>
              </div>
            </div>
            ...
            <div class="my-4 flex items-center gap-3">
              ...
              <span class="text-[10px] font-medium text-slate-400 dark:text-slate-500">CONTEXT</span>
              <div class="h-px flex-1 bg-slate-200 dark:bg-darkmode-600" />
            </div>

            <!-- Selected text -->
            <div>
              <span class="mb-1.5 block text-xs font-medium text-slate-400 dark:text-slate-500">{{ t('semantic-search.selected-context') }}</span>
              <textarea
                :value="allSelectedText"
                readonly
                rows="12"
                class="w-full rounded-xl border border-slate-200 bg-slate-50 p-3 text-xs leading-relaxed text-slate-700 focus:border-primary focus:outline-none focus:ring-2 focus:ring-primary/20 dark:border-darkmode-600 dark:bg-darkmode-700 dark:text-slate-300"
