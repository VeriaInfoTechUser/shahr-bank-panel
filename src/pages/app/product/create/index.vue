      ...
      if (meta.meta_value && meta?.contentOptions?.length) {
        if (meta.meta_key === 'category' && false) {
          ...
          meta.meta_value_object = meta?.contentOptions.find(
              opt => opt.id === meta.meta_value || opt.slug === meta.meta_value
          ) || meta.meta_value_object;
        } else {
          meta.meta_value_object = meta?.contentOptions.find(
              opt => opt.id === meta.meta_value
          );
        }
      }
    }
...
const onMetaValueChange = (meta) => {
  if (meta.meta_key === 'category' && false) {
    const selectedOption = meta?.contentOptions.find(opt => opt.id === meta.meta_value);
    meta.meta_value_object = selectedOption || null;
    meta.meta_value = selectedOption ? selectedOption.slug : '';
  } else {
    meta.meta_value_object = meta?.contentOptions.find(opt => opt.id === meta.meta_value);
  }
}
...
const handleSubmit = async () => {
  ...
  useFetch(`${uri.admin.product.create}`, {
    body: {
      ...submitData
    }
  });
  await router.push({name: 'app-product-detail', params: {slug: submitData.slug}})
...
<template>
  <div class="container mx-auto px-4 py-8 max-w-6xl">
    <div class="flex justify-between items-center mb-6">
      <h1 class="text-2xl font-bold text-gray-800">{{ $t('product.create') }}</h1>
        ...
        <div>
          ...
          <div class="grid grid-cols-2 gap-2">
            <div class="max-w-xl my-2 ps-4">
              <label class="block text-sm font-medium text-gray-700 mb-1">{{ $t('product.slug') }} *</label>
              <input
                  v-model="form.slug"
                  type="text"
