...
// API Calls
const {data: product, error, isFetching, execute: fetchProduct} = useFetch(`${uri.admin.product.get}`, {
  body: {
    slug: props.slug,
    type: "slug"
  }
...
const productMetaOptions = computed(() => {
  return availableMetaOptions.value.filter(meta =>
      meta.option.types.some(type => type.type === 'product')
  );
});
...
watch(product, (newVal) => {
  if (newVal?.data) {
    console.log(newVal.data.status)
    form.value = {
      title: newVal.data.title || '',
      sku: newVal.data.sku || '',
      status: newVal.data.status,
      sub_title: newVal.data.sub_title || '',
      abstract: newVal.data.abstract || '',
      description: newVal.data.description || '',
      related_images: newVal.data.related_images || [],
      meta: newVal.data.meta ? newVal.data.meta.map(m => {
        // For existing category fields, parse the stored object
        if (m.meta_key === 'category' && typeof m.meta_value === 'string' && false) {
          try {
            return {
              ...m,
              meta_value_object: JSON.parse(m.meta_value)
            };
          } catch {
            return m;
          }
      ...
      image: {...newVal.data.image},
      thumbnail: {...newVal.data.thumbnail}
    }

    // Load content for existing meta fields
    loadMetaContent();
  }
}, {immediate: true})
    ...
    if (metaOption?.option.has_content) {
      ...
      if (meta.meta_value && meta?.contentOptions?.length) {
        if (meta.meta_key === 'category' && false) {
          ...
          meta.meta_value_object = meta?.contentOptions.find(
...
const onMetaValueChange = (meta) => {
  if (meta.meta_key === 'category' && false) {
    ...
    meta.meta_value_object = meta?.contentOptions.find(opt => opt.slug === meta.meta_value);
  }
}
...
const handleSubmit = async () => {
  ...
  const {data: product, error, isFetching, execute: fetchProduct} = useFetch(`${uri.admin.product.update}`, {
    body: {
      ...submitData,
      slug: props.slug,
    }
  ...
  setTimeout(()=>
      isLoading.value = false,500)
  // TODO: Send `submitData` to your API
};
...
<template>
  <div class="container mx-auto px-4 py-8 max-w-6xl">
    <div class="flex justify-between items-center mb-6">
      <h1 class="text-2xl font-bold text-gray-800">{{ $t('product.edit_title') }} {{ product?.data?.title }}</h1>
    ...
    <div v-if="isFetching || metaFetching ||isLoading" class="animate-pulse space-y-6 grid grid-cols-1 gap-2">
      <div>
    ...
    >
        ...
        <div>
          ...
          <div class="p-4 rounded-lg">
            <div class="grid grid-cols-1 gap-2">
              <span class="border-b-2 text-xl font-semibold border-blue-500 pb-1">{{ $t('product.images') }}</span>
            </div>
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
              <!-- Main Image -->
              <div class="grid grid-cols-1 gap-2 mt-2">
                <div class="flex flex-row gap-2">
                  <div class="pt-2 w-full">
                    <label class="block text-sm font-medium text-gray-700 mb-2">{{ $t('product.main_image') }}</label>
                  </div>
                  <div>
                    <input
                        type="file"
                  ...
                  <div class="flex-1">
                    <div
                      ...
                      <div v-else class="text-center p-4 text-gray-400">
                        ...
                        <p class="text-sm">{{ $t('product.no_image') }}</p>
                      </div>
                    </div>
                  </div>
                </div>
              ...
              <div class="grid grid-cols-1 gap-2 mt-2">
                <div class="flex flex-row gap-2">
                  <div class="pt-2 w-full">
                    <label class="block text-sm font-medium text-gray-700 mb-2">{{ $t('product.thumbnail') }}</label>
                  </div>
                  <div>
                    <input
                        type="file"
                  ...
                  <div class="flex-1">
                    <div
                      ...
                      <div v-else class="text-center p-4 text-gray-400">
                        ...
                        <p class="text-xs">{{ $t('product.no_image') }}</p>
                      </div>
                    </div>
                  </div>
                </div>
            ...
            <div class="mt-10">
              ...
              <div>
                ...
                <div v-else class="text-center py-4 text-gray-400 text-sm">
                  <p>{{ $t('product.no_related_images') }}</p>
                </div>
              </div>
            </div>
          </div>
      ...
      <div
        ...
        <div class="p-4 rounded-lg grid grid-cols-1 gap-8">
          <div class="grid grid-cols-1 gap-2">
            <span class="border-b-2 text-xl font-semibold border-blue-500 pb-1">{{ $t('product.basic_info') }}</span>
          </div>
          <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
            <div class="grid grid-cols-1 gap-2 ">
              <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">{{ $t('product.title') }} *</label>
                <input
                    v-model="form.title"
                    type="text"
            ...
            <div class="grid grid-cols-1 gap-2 ">
              <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">{{ $t('product.subtitle') }}</label>
                <input
              ...
              <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">{{ $t('product.abstract') }}</label>
                <textarea
      ...
      <div
        ...
        <div class="p-4 rounded-lg grid grid-cols-1 gap-8">
          <div class="flex flex-row border-b-2 border-blue-500">
            <div class="w-full">
              <span class=" text-xl font-semibold pb-1">{{ $t('product.meta_info') }}</span>
            </div>
          </div>
