...
import BaselineFilterPanel from './BaselineFilterPanel.vue';
import {
  BASELINE_FILTER_PARAM_LABEL_KEYS,
  getActiveBaselineFilterKeys,
  type BaselineFilterParamKey,
} from './baselineFilterToolbarKeys';
...
const activeFilterKeys = computed((): BaselineFilterParamKey[] =>
  getActiveBaselineFilterKeys(toValue(props.table.filters) ?? {})
);
...
function removeFilterParam(key: BaselineFilterParamKey) {
...
<template>
  <div class="relative flex flex-shrink-0 flex-wrap items-center gap-1.5" dir="ltr">
    ...
    >
      ...
      >
        ...
        <div class="flex min-w-0 flex-wrap items-center gap-1">
          ...
          >
            <span class="min-w-0 truncate" :title="t(BASELINE_FILTER_PARAM_LABEL_KEYS[key])">
              {{ t(BASELINE_FILTER_PARAM_LABEL_KEYS[key]) }}
            </span>
            <button
              type="button"
    ...
    <!-- Filter popover -->
    <Teleport to="body">
      <Transition name="baseline-filter-pop">
        <div
          v-if="filterOpen"
...
<style scoped>
.baseline-filter-pop-enter-active,
.baseline-filter-pop-leave-active {
  transition: opacity 0.15s ease;
}
.baseline-filter-pop-enter-from,
.baseline-filter-pop-leave-to {
  opacity: 0;
}
</style>
