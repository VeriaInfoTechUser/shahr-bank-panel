...
import ComparativeFilterPanel from './ComparativeFilterPanel.vue';
import {
  COMPARATIVE_FILTER_PARAM_LABEL_KEYS,
  getActiveComparativeFilterKeys,
  type ComparativeFilterParamKey,
} from './comparativeFilterToolbarKeys';
...
const activeFilterKeys = computed((): ComparativeFilterParamKey[] =>
  getActiveComparativeFilterKeys(toValue(props.table.filters) ?? {})
);
...
function removeFilterParam(key: ComparativeFilterParamKey) {
...
<template>
  <div class="relative flex flex-shrink-0 flex-wrap items-center gap-1.5" dir="ltr">
    ...
    >
      ...
      >
        ...
        <div class="flex min-w-0 flex-wrap items-center gap-1">
          ...
          >
            <span class="min-w-0 truncate" :title="t(COMPARATIVE_FILTER_PARAM_LABEL_KEYS[key])">
              {{ t(COMPARATIVE_FILTER_PARAM_LABEL_KEYS[key]) }}
            </span>
            <button
              type="button"
    ...
    <!-- Filter popover -->
    <Teleport to="body">
      <Transition name="comparative-filter-pop">
        <div
          v-if="filterOpen"
...
<style scoped>
.comparative-filter-pop-enter-active,
.comparative-filter-pop-leave-active {
  transition: opacity 0.15s ease;
}
.comparative-filter-pop-enter-from,
.comparative-filter-pop-leave-to {
  opacity: 0;
}
</style>
