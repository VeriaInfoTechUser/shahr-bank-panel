...
import RiskDashboardFilterPanel from './RiskDashboardFilterPanel.vue';
...
function getActiveRiskDashboardFilterKeys(filters: Record<string, unknown>): string[] {
  return Object.keys(filters).filter((key) => key in RISK_DASHBOARD_FILTER_PARAM_LABEL_KEYS);
}
...
<template>
  <div class="relative flex flex-shrink-0 flex-wrap items-center gap-1.5" dir="ltr">
    ...
    >
      ...
      >
        ...
        <div class="flex min-w-0 flex-wrap items-center gap-1">
          ...
          >
            <span class="min-w-0 truncate" :title="t(RISK_DASHBOARD_FILTER_PARAM_LABEL_KEYS[key])">{{
              t(RISK_DASHBOARD_FILTER_PARAM_LABEL_KEYS[key])
            }}</span>
            <button
              type="button"
              ...
              :aria-label="
                t('task.filter-badge-remove-aria', {
                  label: t(RISK_DASHBOARD_FILTER_PARAM_LABEL_KEYS[key]),
                })
              "
              :title="
                t('task.filter-badge-remove-aria', {
    ...
    <Teleport to="body">
      <Transition name="risk-dashboard-filter-pop">
        <div
          v-if="filterOpen"
      ...
      <Transition name="risk-dashboard-filter-pop">
        <div
          ...
          role="dialog"
          aria-modal="true"
          :aria-label="t('risk-dashboard.filter-panel-title')"
          @click.stop
        >
...
<style scoped>
.risk-dashboard-filter-pop-enter-active,
.risk-dashboard-filter-pop-leave-active {
  transition: opacity 0.15s ease;
}
.risk-dashboard-filter-pop-enter-from,
.risk-dashboard-filter-pop-leave-to {
  opacity: 0;
}
</style>
