...
import {
  riskOperationsStatusBadgeClass,
  riskOperationsStatusReportCardClass,
} from '@/composables/riskOperationsStatusBadge';
...
import RiskAssessmentReportDisplay from '@/pages/app/risk/operations/status/RiskAssessmentReportDisplay.vue';
import RiskDoingTaskForm from './RiskDoingTaskForm.vue';

const { t } = useI18n();
const nav = useRiskDoingTaskNavigationStore();
    ...
    if (o.result === false) {
      ...
      throw new Error(
        typeof msg === 'string' && msg.trim()
          ? msg
          : t('risk-operations.doing-task-load-error')
      );
    }
...
const progressRows = computed((): Record<string, unknown>[] => {
  ...
  const pl = o.progress_list;
  const rl = o.risk_list;
  const arr = Array.isArray(pl) ? pl : Array.isArray(rl) ? rl : [];
  return arr.filter((x) => x != null && typeof x === 'object') as Record<
    string,
    unknown
  >[];
});
...
function riskIdsMatch(rowId: unknown, wanted: number | null): boolean {
  if (wanted == null) return false;
  const na = typeof rowId === 'number' ? rowId : Number(rowId);
  const nb = typeof wanted === 'number' ? wanted : Number(wanted);
  if (Number.isFinite(na) && Number.isFinite(nb) && na === nb) return true;
  return String(rowId ?? '') === String(wanted ?? '');
}

/**
 * فرم: task + risk (همان قرارداد ردیف عملیات ریسک).
 */
const doingAnswerFormRow = computed((): Record<string, unknown> | null => {
  ...
  if (rows.length > 0) {
    if (pid != null) {
      p = rows.find((r) => riskIdsMatch(r.id, pid));
    }
    if (!p) {
      p = rows[0];
    }
  } else {
    const nested = tk.risk;
  ...
  const synthetic: Record<string, unknown> = { task: tk, risk: p };
  const tid = resolveOperationsTaskRowId(synthetic);
  if (tid != null) {
    synthetic.id = tid;
  }
  return synthetic;
...
const assignerDisplayLabel = computed(() => {
  ...
  if (formRow) {
    const risk = formRow.risk as Record<string, unknown> | undefined;
    if (risk) fromApi = assignerLabelFromProgressPayload(risk).trim();
  }
...
function progressStatusBadgeClass(p: Record<string, unknown>): string {
  return riskOperationsStatusBadgeClass(progressStatusKey(p));
}
...
/** وضعیت تأیید نهایی — گزارش ارزیابی بدون حالت غیرفعال */
function isRiskProgressApproved(p: Record<string, unknown>): boolean {
  return progressStatusKey(p) === 'approve';
}
...
async function loadDetails() {
  ...
  async function fetchTaskGetOnly(): Promise<boolean> {
    try {
      ...
      const msg =
        e instanceof Error ? e.message : t('risk-operations.doing-task-load-error');
      loadError.value = msg;
  ...
  if (pid != null) {
    ...
    try {
      const progRaw = await ermRepo.riskProgressDetail({
        task_id: tid,
        progress_id: pid,
      });
      ...
      const msg =
        e instanceof Error ? e.message : t('risk-operations.doing-task-load-error');
      toast(
        `${t('risk-operations.doing-task-section-progress')}: ${msg}`,
        { type: 'error' }
      );
    }
...
<template>
  <div class="grid grid-cols-12 gap-3 p-2">
    <div class="col-span-12">
      ...
      >
        {{ t('risk-operations.doing-task-page-no-context') }}
      </p>
        ...
        >
            ...
            >
              ...
              >
                ...
                >
                  ...
                  >
                    {{ t('risk-operations.doing-task-assigner-label') }}
                  </span>
          ...
          <template v-if="hasProgressDashboard">
            <section v-if="reportSummary">
              ...
              <div
                class="grid grid-cols-2 gap-1.5 sm:grid-cols-3 lg:grid-cols-6"
              >
                <div
                  v-for="k in REPORT_DASHBOARD_KEYS"
                  :key="k"
                  :class="[
                    riskOperationsStatusReportCardClass(k),
                    'px-2 py-1.5 shadow-none',
                  ]"
                >
                  <p
                    class="text-[9px] font-medium leading-tight opacity-90"
                  >
                ...
                <div
                  class="min-w-0 rounded-md border border-y border-l border-r-[3px] border-r-slate-300/80 border-slate-200/80 bg-slate-50/80 px-2 py-1.5 text-slate-800 dark:border-darkmode-600 dark:border-r-slate-600 dark:bg-darkmode-900/35 dark:text-slate-100"
                >
                  <p
                    class="text-[9px] font-medium leading-tight text-slate-500 dark:text-slate-400"
                  >
                    {{ t('risk-operations.doing-task-report-average') }}
                  </p>
                  <p class="text-sm font-semibold tabular-nums leading-tight">
            ...
            >
              {{ t('risk-operations.doing-task-no-report-block') }}
            </p>
            ...
            <section>
              ...
              <div class="space-y-3">
                ...
                <p
                  ...
                  class="rounded-md border border-dashed border-slate-200/90 px-3 py-4 text-center text-[11px] text-slate-500 dark:border-darkmode-600 dark:text-slate-400"
                >
                  {{ t('risk-operations.doing-task-empty-progress-list') }}
                </p>
              </div>
            </section>
          </template>
          ...
          >
            {{ t('risk-operations.doing-task-no-progress-detail') }}
          </p>
        </div>
      </template>
    </div>
