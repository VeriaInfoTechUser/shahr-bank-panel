...
import RiskDashboardFilterToolbar from '../dashboard_old/RiskDashboardFilterToolbar.vue';
...
interface RiskPerformanceResponse {
...
const insights = computed(() => [
  {
    icon: 'Layers3',
    label: t('risk-performance.total-workload'),
    value: t('risk-performance.items-count', { count: totalTracked.value }),
  },
  {
    icon: 'TrendingUp',
    label: t('risk-performance.approval-rate'),
    value: t('risk-performance.percent-value', { percent: approvalRate.value }),
  },
  {
    icon: 'Timer',
    label: t('risk-performance.current-bottleneck'),
    value: busiestStatus.value?.title || '-',
  },
  {
    icon: 'CheckCheck',
    label: t('risk-performance.finalized-items'),
    value: t('risk-performance.items-count', { count: totalDone.value + totalApproved.value + totalRejected.value }),
  },
]);
  ...
  <div class="mx-auto max-w-7xl px-1 pb-12 pt-2 md:px-2">
    ...
    >
      ...
      <h2 class="mb-2 font-semibold text-danger">
        {{ t('risk-performance.error-title') }}
      </h2>
      <p class="mb-6 text-sm text-slate-600 dark:text-slate-300">
        {{ error.message || t('risk-performance.error-message') }}
      </p>
        ...
        >
          ...
          <div class="relative mt-3 flex items-center justify-between px-6 pb-6 text-sm">
            <span class="font-medium text-slate-500 dark:text-slate-400">
              {{ t('risk-performance.percent-label') }}
            </span>
      ...
      <div class="rounded-2xl border border-slate-200/90 bg-white p-6 shadow-sm dark:border-darkmode-600 dark:bg-darkmode-800">
        <div class="mb-5">
          <h2 class="text-base font-semibold text-slate-900 dark:text-slate-50">
            {{ t('risk-performance.status-list-title') }}
          </h2>
    ...
    >
      ...
      <h2 class="mb-2 font-semibold text-slate-900 dark:text-slate-50">
        {{ t('risk-performance.no-data-title') }}
      </h2>
      <p class="text-sm text-slate-500 dark:text-slate-400">
        {{ t('risk-performance.no-data-message') }}
      </p>
    </div>
  </div>
</template>
