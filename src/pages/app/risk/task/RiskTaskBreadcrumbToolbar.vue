...
import RiskTaskFilterPanel from './RiskTaskFilterPanel.vue';
import {
  RISK_TASK_FILTER_PARAM_LABEL_KEYS,
  getActiveRiskTaskFilterKeys,
  type RiskTaskFilterParamKey,
} from './riskTaskFilterToolbarKeys';
...
const activeFilterKeys = computed((): RiskTaskFilterParamKey[] =>
  getActiveRiskTaskFilterKeys(toValue(props.table.filters) ?? {})
);
...
function removeFilterParam(key: RiskTaskFilterParamKey) {
...
<template>
  <div class="relative flex flex-shrink-0 flex-wrap items-center gap-1.5" dir="ltr">
    <button
      ...
      :aria-label="t('risk-task.toolbar-export')"
      :title="t('risk-task.toolbar-export')"
    ...
    >
      <div class="flex shrink-0 items-center gap-1">
        <button
          ...
          :aria-label="t('risk-task.toolbar-filter')"
          :aria-expanded="filterOpen"
          :title="t('risk-task.toolbar-filter')"
          @click="toggleFilter"
        >
        ...
        <button
          ...
          :aria-label="t('risk-task.toolbar-clear-filters')"
          :title="t('risk-task.toolbar-clear-filters')"
          @click="clearFiltersFromToolbar"
        >
          <span class="truncate">{{ t('risk-task.toolbar-clear-filters') }}</span>
        </button>
      </div>
      <div
      ...
      >
        ...
        <div class="flex min-w-0 flex-wrap items-center gap-1">
          ...
          >
            <span class="min-w-0 truncate" :title="t(RISK_TASK_FILTER_PARAM_LABEL_KEYS[key])">{{
              t(RISK_TASK_FILTER_PARAM_LABEL_KEYS[key])
            }}</span>
            <button
              type="button"
              ...
              :aria-label="
                t('risk-task.filter-badge-remove-aria', { label: t(RISK_TASK_FILTER_PARAM_LABEL_KEYS[key]) })
              "
              :title="
                t('risk-task.filter-badge-remove-aria', { label: t(RISK_TASK_FILTER_PARAM_LABEL_KEYS[key]) })
              "
    ...
    <Teleport to="body">
      <Transition name="risk-task-filter-pop">
        <div
          v-if="filterOpen"
      ...
      <Transition name="risk-task-filter-pop">
        <div
          ...
          role="dialog"
          aria-modal="true"
          :aria-label="t('risk-task.filter-panel-title')"
          @click.stop
        >
          <RiskTaskFilterPanel
            :table="table"
...
<style scoped>
.risk-task-filter-pop-enter-active,
.risk-task-filter-pop-leave-active {
  transition: opacity 0.15s ease;
}
.risk-task-filter-pop-enter-from,
.risk-task-filter-pop-leave-to {
  opacity: 0;
}
</style>
