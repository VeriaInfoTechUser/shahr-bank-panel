...
import CapabilityFilterPanel from './CapabilityFilterPanel.vue';
import {
  CAPABILITY_FILTER_PARAM_LABEL_KEYS,
  getActiveCapabilityFilterKeys,
  type CapabilityFilterParamKey,
} from './capabilityFilterToolbarKeys';
...
const activeFilterKeys = computed((): CapabilityFilterParamKey[] =>
  getActiveCapabilityFilterKeys(toValue(props.table.filters) ?? {})
);
...
function removeFilterParam(key: CapabilityFilterParamKey) {
...
<template>
  <div class="relative flex flex-shrink-0 flex-wrap items-center gap-1.5" dir="ltr">
    <button
      ...
      :aria-label="t('sustainability-capability-page.toolbar-add')"
      :title="t('sustainability-capability-page.toolbar-add')"
      @click="props.onAdd?.()"
    >
    ...
    <button
      ...
      :aria-label="t('sustainability-capability-page.toolbar-export')"
      :title="t('sustainability-capability-page.toolbar-export')"
    ...
    >
      <div class="flex shrink-0 items-center gap-1">
        <button
          ...
          :aria-label="t('sustainability-capability-page.toolbar-filter')"
          :aria-expanded="filterOpen"
          :title="t('sustainability-capability-page.toolbar-filter')"
          @click="toggleFilter"
        >
        ...
        <button
          ...
          :aria-label="t('sustainability-capability-page.toolbar-clear-filters')"
          :title="t('sustainability-capability-page.toolbar-clear-filters')"
          @click="clearFiltersFromToolbar"
        >
          <span class="truncate">{{ t('sustainability-capability-page.toolbar-clear-filters') }}</span>
        </button>
      </div>
      <div
      ...
      >
        ...
        <div class="flex min-w-0 flex-wrap items-center gap-1">
          ...
          >
            <span class="min-w-0 truncate" :title="t(CAPABILITY_FILTER_PARAM_LABEL_KEYS[key])">{{
              t(CAPABILITY_FILTER_PARAM_LABEL_KEYS[key])
            }}</span>
            <button
              type="button"
              ...
              :aria-label="
                t('sustainability-capability-page.filter-badge-remove-aria', { label: t(CAPABILITY_FILTER_PARAM_LABEL_KEYS[key]) })
              "
              :title="
                t('sustainability-capability-page.filter-badge-remove-aria', { label: t(CAPABILITY_FILTER_PARAM_LABEL_KEYS[key]) })
              "
    ...
    <Teleport to="body">
      <Transition name="capability-filter-pop">
        <div
          v-if="filterOpen"
      ...
      <Transition name="capability-filter-pop">
        <div
          ...
          role="dialog"
          aria-modal="true"
          :aria-label="t('sustainability-capability-page.filter-panel-title')"
          @click.stop
        >
...
<style scoped>
.capability-filter-pop-enter-active,
.capability-filter-pop-leave-active {
  transition: opacity 0.15s ease;
}
.capability-filter-pop-enter-from,
.capability-filter-pop-leave-to {
  opacity: 0;
}
</style>
