...
import IndicatorFilterPanel from './IndicatorFilterPanel.vue';
import {
  INDICATOR_FILTER_PARAM_LABEL_KEYS,
  getActiveIndicatorFilterKeys,
  type IndicatorFilterParamKey,
} from './indicatorFilterToolbarKeys';
...
const activeFilterKeys = computed((): IndicatorFilterParamKey[] =>
  getActiveIndicatorFilterKeys(toValue(props.table.filters) ?? {})
);
...
function removeFilterParam(key: IndicatorFilterParamKey) {
...
<template>
  <div class="relative flex flex-shrink-0 flex-wrap items-center gap-1.5" dir="ltr">
    ...
    >
      ...
      >
        ...
        <div class="flex min-w-0 flex-wrap items-center gap-1">
          ...
          >
            <span class="min-w-0 truncate" :title="t(INDICATOR_FILTER_PARAM_LABEL_KEYS[key])">{{
              t(INDICATOR_FILTER_PARAM_LABEL_KEYS[key])
            }}</span>
            <button
              type="button"
    ...
    <Teleport to="body">
      <Transition name="indicator-filter-pop">
        <div
          v-if="filterOpen"
...
<style scoped>
.indicator-filter-pop-enter-active,
.indicator-filter-pop-leave-active {
  transition: opacity 0.15s ease;
}
.indicator-filter-pop-enter-from,
.indicator-filter-pop-leave-to {
  opacity: 0;
}
</style>
